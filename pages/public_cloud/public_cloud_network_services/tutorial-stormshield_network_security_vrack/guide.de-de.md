...
Log into the [OVHcloud Control Panel](/links/manager), go to the `Public Cloud`{.action} section, and select the relevant Public Cloud project. In the left menu, click on `Instanzen`{.action} under the **Compute** tab, then find your two SNS EVA instances.
